...
export default function MarketplaceProviderDetailPageClient({ id }: { id: string }) {
                ...
                >
                  ...
                  <div className="p-4">
                    <div className="flex items-center gap-2">
                      ...
                      {provider.verification_badge && (
                        <SealCheck size={14} className="shrink-0 text-accent" weight="fill" />
                      )}
                    </div>
                ...
                >
                        ...
                        <div className="flex items-center gap-2 text-[11px] text-muted">
                          {service.contract_type && (
                            <span className="rounded bg-white px-1.5 py-0.5 text-[10px] font-medium">
                              {service.contract_type.replace(/_/g, " ")}
                            </span>
                          )}
                          {service.pricing_model && (
                            <span className="capitalize">{service.pricing_model.replace(/_/g, " ")}</span>
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="p-4 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1.5 text-[11px] text-muted">
                      {service.coverage_area?.length > 0 && (
                        <span className="flex items-center gap-1 rounded-full bg-blue-50 px-1.5 py-0.5 font-medium text-blue-600">
                          <MapPin size={10} /> {service.coverage_area[0]}{service.coverage_area.length > 1 ? ` +${service.coverage_area.length - 1}` : ""}
                        </span>
                      )}
                      {service.estimated_response_time && (
                        <span className="rounded-full bg-amber-50 px-1.5 py-0.5 font-medium text-amber-700">
                          Resp: {service.estimated_response_time}
                        </span>
                      )}
                      {service.certifications_or_licenses?.length > 0 && (
                        <span className="rounded-full bg-green-50 px-1.5 py-0.5 font-medium text-green-600">
                          {service.certifications_or_licenses[0]}{service.certifications_or_licenses.length > 1 ? " +" : ""}
                        </span>
                      )}
                    </div>
                    {service.availability_status && (
                      <p className={`text-[10px] font-medium ${
                        service.availability_status === "active" ? "text-green-600" : "text-amber-600"
                      }`}>
                        {service.availability_status === "active" ? "Available now" : "Limited availability"}
                      </p>
                    )}
                    ...
                    <Link
                      ...
                      className="mt-2 block w-full rounded-lg border border-accent/20 px-3 py-1.5 text-center text-xs font-medium text-accent hover:bg-accent/5 transition-colors"
                    >
